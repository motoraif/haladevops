...
// Animated Page Hero Background (for services, case-studies, about pages)
const pageHeroCanvas = document.getElementById('page-hero-canvas');
if (pageHeroCanvas) {
    const ctx = pageHeroCanvas.getContext('2d');
    let particles = [];
    let animationId;

    function resizeCanvas() {
        pageHeroCanvas.width = pageHeroCanvas.offsetWidth;
        pageHeroCanvas.height = pageHeroCanvas.offsetHeight;
    }

    class Particle {
        constructor() {
            this.x = Math.random() * pageHeroCanvas.width;
            this.y = Math.random() * pageHeroCanvas.height;
            this.size = Math.random() * 3 + 1;
            this.speedX = Math.random() * 0.5 - 0.25;
            this.speedY = Math.random() * 0.5 - 0.25;
            this.opacity = Math.random() * 0.5 + 0.2;
        ...
        update() {
            ...
            if (this.x > pageHeroCanvas.width) this.x = 0;
            if (this.x < 0) this.x = pageHeroCanvas.width;
            if (this.y > pageHeroCanvas.height) this.y = 0;
            if (this.y < 0) this.y = pageHeroCanvas.height;
        }
    ...
    function init() {
        resizeCanvas();
        particles = [];
        const particleCount = Math.floor((pageHeroCanvas.width * pageHeroCanvas.height) / 15000);
        for (let i = 0; i < particleCount; i++) {
            particles.push(new Particle());
        }
    }
    ...
    function animate() {
        ctx.clearRect(0, 0, pageHeroCanvas.width, pageHeroCanvas.height);
